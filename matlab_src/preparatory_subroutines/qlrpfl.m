function prop = qlrpfl(prop)
% QLRPFL - Preparatory subroutine for point-to-point mode
%
...
%   prop - Structure containing all input and output propagation parameters
    ...
    [prop.the, prop.dl] = hzns(prop.pfl, prop.dist, prop.hg, prop.gme);
    ...
    if prop.dl(1) + prop.dl(2) >= 1.5 * prop.dist
        ...
        prop.he(2) = prop.hg(2) + max(prop.pfl(np+2) - zb, 0);
        ...
        [za, ~] = zlsq1(prop.pfl, xl(1), 0.9 * prop.dl(1));
        [~, zb] = zlsq1(prop.pfl, prop.dist - 0.9 * prop.dl(2), xl(2));
