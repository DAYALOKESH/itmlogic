function [avar1, prop] = avar(zzt, zzl, zzc, prop)
% AVAR - Attenuation quantile computation
%
% Finds the quantiles of attenuation using the output from lrprop (Section V of "The ITS
% Irregular Terrain Model, version 1.2.2: The Algorithm").
%
...
%
% Parameters:
%   zzt - Standard normal deviate corresponding to user defined quantile
%   zzl - Standard normal deviate corresponding to user defined quantile
%   zzc - Standard normal deviate corresponding to user defined quantile
%   prop - Structure containing all input propagation parameters
%
% Returns:
%   avar1 - Additional attenuation from the median
%   prop - Structure containing all input and output propagation parameters
    ...
    if prop.lvar > 0
        ...
        if prop.lvar > 2
            ...
            prop.gm = prop.cfm1 + prop.cfm2 / ((prop.cfm3 * q)^2 + 1);
            
            prop.gp = prop.cfp1 + prop.cfp2 / ((prop.cfp3 * q)^2 + 1);
        end
        
        if prop.lvar > 1
            prop.dexa = sqrt(18e6 * prop.he(1)) + sqrt(18e6 * prop.he(2)) + ...
        ...
        prop.sgtm = curv(prop.csm1, prop.csm2, prop.ysm1, prop.ysm2, ...
            prop.ysm3, de) * prop.gm;
        
        prop.sgtp = curv(prop.csp1, prop.csp2, prop.ysp1, prop.ysp2, ...
            prop.ysp3, de) * prop.gp;
