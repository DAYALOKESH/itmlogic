...
% LRPROP - Basic Longley-Rice propagation program
%
...
%          including the reference attenuation (aref)
    ...
    if prop.mdp ~= 0
        ...
        if prop.hg(2) < 1 || prop.hg(2) > 1000
            prop.kwx = max(prop.kwx, 1);
        end
        
        if abs(prop.the(1)) > 0.2 || prop.dl(1) < 0.1 * prop.dls(1) || ...
                prop.dl(1) > 3 * prop.dls(1)
            prop.kwx = max(prop.kwx, 3);
        end
        
        if abs(prop.the(2)) > 0.2 || prop.dl(2) < 0.1 * prop.dls(2) || ...
                prop.dl(2) > 3 * prop.dls(2)
            prop.kwx = max(prop.kwx, 3);
        end
        
        if prop.ens < 250 || prop.ens > 400 || prop.gme < 75e-9 || ...
                prop.gme > 250e-9 || real(prop.zgnd) < abs(imag(prop.zgnd)) || ...
        ...
        if prop.hg(2) < 0.5 || prop.hg(2) > 3000
            prop.kwx = 4;
        end
    ...
    if prop.dist < prop.dlsa
        if prop.wlos == 0
            ...
            if d0 < d1
                a0 = alos(d0, prop);
                q = log(d2 / d0);
                prop.ak2 = max(0, ((d2 - d0) * (a1 - a0) - (d1 - d0) * ...
                    (a2 - a0)) / ((d2 - d0) * log(d1 / d0) - (d1 - d0) * q));
            ...
            if prop.dist > 0
                prop.aref = prop.ael + prop.ak1 * prop.dist + ...
                    prop.ak2 * log(prop.dist);
            end
        end
    end
    ...
    if prop.dist <= 0 || prop.dist >= prop.dlsa
        if prop.wscat == 0
            ...
            if a5 < 1000
                ...
                prop.dx = max([prop.dlsa, prop.dla + 0.3 * prop.xae * ...
                    log(47.7 * prop.wn), (a5 - prop.aed - prop.ems * d5) / ...
                    (prop.emd - prop.ems)]);
